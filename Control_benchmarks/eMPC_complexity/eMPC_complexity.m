...
xmin = -20;
xmax = 50;
dmax = 2*max(disturbances')';
dmin = min(disturbances')' - 10*ones(nd,1);
% prediction horizon
N = 8;
Nc = 1;
...
sx = sdpvar(nx, N, 'full');          % decision variables - slack for input constraints

run_eMPC = 1;  % if 0, script runs iMPC 
...
%     con = con + [ 0 <=  su(:, k) <= 2*umax ];
%     con = con + [ 0 <=  sy(:, k) <= 2*xmax ];
%     con = con + [ xmin <= x(:, k) <= xmax];
%     con = con + [ 0 <=  sx(:, k) <= 2*xmax ];
%     con = con + [ 0 <=  su(:, k)];
%     con = con + [ 0 <=  sy(:, k)];
%     con = con + [ 0 <=  sx(:, k)];
%     con = con + [ xmin - sx(:, k) <= x(:, k) <= xmax + sx(:, k)];


    if run_eMPC
        con = con + [ xmin <= yref(:, k) <= xmax ];
        con = con + [ umin <= u(:, k) <= umax ];                    % input constraints
%         con = con + [ dmin <= d(:, k) <= dmax ];
%         con = con + [ 0 <=  su(:, k) ];
        con = con + [ 0 <=  sy(:, k) ];
% %         con = con + [ xmin <= x(:, k) <= xmax];  
    else
        ...
        con = con + [ xmin <= x(:, k) <= xmax];
        con = con + [ 0 <=  su(:, k) <= 2*umax ];
        con = con + [ umin - su(:, k) <= u(:, k) <= umax + su(:, k) ]; 
    end  
...
if run_eMPC
    % eMPC optimizer
    plp = Opt(con, obj, [x(:, 1); yref(:); d(:)], u(:, 1));
    solution = plp.solve();
    filename = strcat('eMPC_policy',num2str(N));
...
R_day = R_day(1:end-1);
R_t = repmat(R_day, 1, end_day+1); %  Sim_days control profile   
% simulation disturbance and reference trajectories
d_prew = disturbances(:,start_sim:end_sim+N);
r_prew = R_t(:,start_sim:end_sim+N);
...
%             uopt = eMPC_policy(t0);
            filename = strcat('eMPC_policy',num2str(N),'(t0)');
            u_sol= eval(filename);
%             u_sol = solution.xopt.feval(t0, 'primal');
...
if run_eMPC %drom fist run due to the file load
    CPU_time_max_paper = max(CPU_time_max(2:end))*1000
else
